...
Hooks.on('updateCombat', async (combat, updateData, options, userId) => {
    ...
    if ('round' in updateData) {
        console.log(
            `Ilaris Alternative Actor Sheet | Combat update hook triggered on ${game.user.name} (isGM: ${game.user.isGM}) - Round: ${combat.round}`
        );
        ...
        if (game.user.isGM) {
            ...
            if (updates.length > 0) {
                console.log(
                    'Ilaris Alternative Actor Sheet | Applying initiative resets for',
                    updates.length,
                    'combatants'
                );
...
Hooks.on('Ilaris.postAngriff', async (rollResult, dialog) => {
    // Only process ranged attacks on the new unified attack hook
    if (dialog?.attackType !== 'ranged') {
        return;
    }

    const actor = dialog?.actor ?? dialog?.sourceActor ?? dialog?.attacker ?? null;
    const item = dialog?.item ?? dialog?.weapon ?? dialog?.angriffsItem ?? null;

    if (!actor || !item) {
        console.warn('Ilaris Alternative Actor Sheet | Missing actor or item on Ilaris.postAngriff ranged hook', {
    ...
    const ammoProperty = item.system.eigenschaften?.find(e => AMMUNITION_TYPES.includes(e.key));
        ...
        onChange: async () => {
            try {
                ...
                if (stats.actorsProcessed > 0) {
                    ...
                    console.log(
                        `Ilaris Alternative Actor Sheet | Advanced time: ${stats.actorsProcessed} actors, ${stats.effectsReduced} effects reduced`
                    );
                } else {
