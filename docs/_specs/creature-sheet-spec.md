...
<div class='uebernatuerlich statblockreihe'>
    ...
    {{#each actor.uebernatuerlich.fertigkeiten as |fertigkeit|}}
        <a class='item-edit' data-itemid='{{fertigkeit.id}}'>{{fertigkeit.name}}</a>
        ({{fertigkeit.system.pw}}
        <a
            class='rollable icon-small'
            data-rolltype='fertigkeit_diag'
            data-probetype='simple'
            data-fertigkeit='{{fertigkeit.name}}'
            data-pw='{{fertigkeit.system.pw}}'
        >
            <i class='fas fa-dice-d20'></i>
        </a>),
    {{/each}}
