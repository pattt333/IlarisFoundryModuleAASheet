...
GitHub Actions workflow triggered on `v*` tags that creates `module.zip` with all module files (including pre-compiled `packs/`), updates `module.json` manifest URLs to the release tag, and publishes a GitHub Release.
