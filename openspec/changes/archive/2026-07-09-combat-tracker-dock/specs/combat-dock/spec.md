...
A horizontal carousel combat tracker displaying all combatants as cards at the top or bottom of the screen, replacing the visual role of Foundry's sidebar tracker with a more engaging, always-visible HUD.
...
When initiative has been rolled and combat is in progress, the dock SHALL display the current combatant's card at a larger size (approximately 170px) with a pulse animation, while all other cards remain at a smaller size (approximately 120px).

#### Scenario: Turn changes to a new combatant

- **WHEN** `combatTurn` fires and a new combatant becomes active
- **THEN** the previous combatant's card shrinks to standard size and loses the pulse animation, the new current combatant's card enlarges and the pulse animation begins
...
### Requirement: Carousel with scroll buttons

The dock SHALL display the current combatant's card centered with up to 2 cards visible on each side. When the total combatant count exceeds this visible window (more than 5 combatants), scroll buttons SHALL appear at the left and right edges. Clicking a scroll button SHALL shift the visible window by one card.

#### Scenario: Five or fewer combatants

- **WHEN** combat has 5 or fewer combatants
- **THEN** all cards are visible, no scroll buttons appear

#### Scenario: More than five combatants

- **WHEN** combat has 8 combatants
- **THEN** the current combatant plus 2 cards on each side are visible (5 total), and scroll buttons appear at both edges

#### Scenario: Current combatant is first in order

- **WHEN** the first combatant in initiative order is the current turn
- **THEN** the right scroll button is visible to scroll forward, and no cards are visible to the left of the current

#### Scenario: Current combatant is last in order

- **WHEN** the last combatant in initiative order is the current turn
- **THEN** the left scroll button is visible to scroll backward, and no cards are visible to the right of the current

#### Scenario: Click scroll button

- **WHEN** the user clicks the right scroll button
- **THEN** the carousel scrolls to show the next card, shifting the visible window by one position

### Requirement: Auto-scroll on turn change

When the combat turn changes, the carousel SHALL automatically scroll to center the new current combatant's card.

#### Scenario: Turn advances forward

- **WHEN** combat turn advances to the next combatant
- **THEN** the carousel smoothly scrolls to center the new current card in the visible window

#### Scenario: New round begins

- **WHEN** a new round starts and the first combatant becomes active
- **THEN** the carousel scrolls to center the first combatant's card
...
- **THEN** a single card is rendered centered, no scroll buttons appear
...
- **THEN** the carousel displays the current ±2 window with scroll buttons, and performance remains acceptable
