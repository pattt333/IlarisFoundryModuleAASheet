...
An AppV2 dialog (`IlarisAlternativeAiCreatureDialog`) SHALL provide: a Stärke dropdown (schwach/mittel/stark/boss), a Gruppengröße input (1-10), a Typ dropdown (humanoid/bestie/dämon/untoter/beliebig), and a textarea for natural language description. A "Generieren" button SHALL trigger the API call.
...
- **WHEN** a GM clicks the "KI-Kreaturen-Generator" button in the actor directory
- **THEN** the generator dialog opens

### Requirement: Token-optimized prompt sent to DeepSeek API

The prompt SHALL include: a compact JSON schema of the Kreatur data model, a strength-to-attribute-range lookup table, a filtered list of creature-relevant vorteile names by category, damage formula syntax (`NdN+N`), weapon property keys, creature type options, two few-shot examples (mittel humanoid, stark bestie), and the user's natural language description. Total prompt SHALL be under 2,000 tokens.

#### Scenario: Prompt constructed for generation

- **WHEN** the user clicks "Generieren"
- **THEN** a system prompt with schema, strength table, vorteile cache, and examples is combined with the user's description and sent to `https://api.deepseek.com/v1/chat/completions` with model `deepseek-chat`
...
Each generated creature SHALL be validated: attributes clamped to 8-20, Kampfwerte to reasonable bounds (HP 1-200, INI 1-30, etc.), weapon damage formulas validated against the `NdN+N` pattern, vorteile checked against the cached list, and missing required fields filled with Kreatur data model defaults.

#### Scenario: AI returns attribute value of 25

- **WHEN** a generated creature has `KL.wert: 25`
- **THEN** the value is clamped to 20

#### Scenario: AI returns invalid damage formula

- **WHEN** a generated weapon has `tp: "fire damage"`
- **THEN** the weapon's tp is replaced with a default `"1d6"`

### Requirement: Creature actors created with embedded weapons

After validation, each creature in the response SHALL be created via `Actor.create({ type: 'kreatur', name, system: {...} })`. Weapons from the AI response SHALL be created as embedded `angriff` items. A notification SHALL confirm the number of creatures created.
...
- **THEN** an error notification is shown with the error message, and the dialog remains open for retry

### Requirement: Vorteile cache refreshable via settings

A button in module settings SHALL read the Ilaris system's vorteile compendium, extract creature-relevant vorteile (categories: allgemein, profan, kampf, kampfstil), and store the filtered names as a JSON cache in `game.settings`. The cache SHALL be used by the prompt builder.

#### Scenario: GM clicks "Vorteile-Cache aktualisieren"

- **WHEN** the GM clicks the cache refresh button in module settings
- **THEN** the vorteile compendium is read, filtered to creature-relevant categories, and stored as a JSON cache
