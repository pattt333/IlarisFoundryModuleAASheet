...
const TRANSLATIONS = {
    COMMON_ITEM: {
        ...
        CHIRURGISCHE_INSTRUMENTE_HOCHWERTIGES_INSTRUMENTARIUM:
            'Chirurgische Instrumente (Hochwertiges Instrumentarium)',
        DIETRICHE: 'Dietriche',
        ...
        STORCHENMASKE_ZUBEHOER_GESUNDHEITSSTEIGERNDE_ESSENZEN_1_ANWENDUNG:
            'Storchenmaske-Zubehör Gesundheitssteigernde Essenzen (1 Anwendung)',
    ...
    COMMON_ITEM_DESCRIPTION: {
        ...
        BERSERKERELEXIER_DESCRIPTION:
            'Der Trinkende verfällt in einen Kampfrausch. Er erhält die Vorteile Kalte Wut und Offensiver Kampfstil und nutzt in jeder Aktion volle Offensive. Sind keine Gegner mehr da, greift er auch Verbündete an.',
        ...
        EINBEERENTRANK_DESCRIPTION:
            'Während einer Rast von mindestens einer Stunde regeneriert der Trinkende eine Wunde. Bei mehr als einer Einnahme pro Tag droht die Einbeerensucht (1-8 auf dem W20).',
        ELIXIERE_DER_TUGEND_DESCRIPTION:
            'Für 1 Stunde sind Proben auf ein bestimmtes Attribut um +2, Fertigkeitsproben mit diesem Attribut um +1 erleichtert.',
        ERSCHOEPFUNGSPILLEN_DESCRIPTION: 'Heilen sofort 2 Punkte Erschöpfung.',
        ESSBESTECK_DESCRIPTION: 'Keine Beschreibung',
        EULENTRAENEN_DESCRIPTION:
        ...
        HYLAILER_FEUER_DESCRIPTION:
            'Die ölige Flüssigkeit entzündet sich, wenn ihr Behältnis zerbricht und ist nicht mit Wasser zu löschen. Bei 1–15 auf dem 1W20 versagt das Hylailer Feuer.',
        ...
        UNSICHTBARKEITSELEXIER_DESCRIPTION:
...
function translateItemData(originalItem) {
    ...
    if (translatedItem.name && TRANSLATIONS.COMMON_ITEM[translatedItem.name]) {
        console.log(
            `   Übersetze Name: "${translatedItem.name}" -> "${TRANSLATIONS.COMMON_ITEM[translatedItem.name]}"`
        );
...
async function convertToFoundryV12Items(inputFilePath, outputFolder) {
    try {
        ...
        console.log(
            `Übersetzungsladen: ${Object.keys(TRANSLATIONS.COMMON_ITEM).length} Namen, ${Object.keys(TRANSLATIONS.COMMON_ITEM_DESCRIPTION).length} Beschreibungen\n`
        );
